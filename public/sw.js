 ...
 *
 * Em desenvolvimento (localhost/127.0.0.1) o SW auto-desregistra e limpa
 * todos os caches para evitar conflitos com o HMR do Vite.
 */
...
if (isDev) {
  // Auto-desregistro em dev: remove o SW e todos os caches, depois recarrega
  // os clients para que a página passe a operar sem interferência do cache.
  ...
  // Não registra nenhum handler de fetch/message/push em dev — o SW já foi removido.
} else {
  self.addEventListener('install', (event) => {
    event.waitUntil(
      caches
  ...
  self.addEventListener('fetch', (event) => {
    ...
    // Exclui paths do Vite dev server (/src/, @vite, @react-refresh, ?t=, ?import=)
    // para não cachear módulos que mudam a cada HMR.
    ...
    if (url.origin === self.location.origin && !isViteDevAsset) {
      event.respondWith(
